...
export function WhatsAppButton({ message, label = 'CHAT ON WHATSAPP', className = '', phoneDigits }: WhatsAppButtonProps) {
  ...
  return (
    <a
      className={`inline-flex items-center gap-2 px-6 py-3.5 bg-oxblood hover:bg-ink text-paper text-xs font-bold tracking-widest uppercase rounded-sm shadow transition-all ${className}`}
      href={href}
      target="_blank"
      rel="noopener noreferrer"
    >
      <svg className="w-4 h-4 fill-brass" viewBox="0 0 24 24">
